...
public class CSListingDocumentFactory {
	...
	public static Document createDocument(CSListing poi) {
		...
		document.add(new Field(DIRECTORY_LISTING_ID, "cs" + poi.getListingId() + (poi.getWhereId() != null ? " w" + poi.getWhereId() : "") + (poi.getYpId() != null ? " yp" + poi.getYpId() : "") + (poi.getSpId() != null ? " sp" + poi.getSpId() : ""), Field.Store.NO, Field.Index.ANALYZED));
	...
	private static void addLocationFields(CSListing poi, Document doc)
	{
		if(poi == null) {return;}		
		Location addr = poi.getAddress();
		if(addr == null){return;}
	...
	public static void toLocationWords(CSListing poi, PrintWriter writer) {
	...
	public static void toWords(CSListing poi, PrintWriter writer) {
	...
	public static Document createCategoryDocument(CSListing poi) {
	...
	private static void addCSListing(CSListing poi, Document document) {
		try {
	...
	public static CSListing createCSListing(Document document, int documentId) {
		return createCSListing(document, documentId, null, false);
	}
	
	public static CSListing createCSListing(Document document, int documentId, ICache cache, boolean checkcache) {
		if(document == null) return null;
		try {
			CSListing poi = null;
			if(checkcache && cache != null) {
				Object cachedob = cache.getObject(""+documentId);
				if(cachedob != null) poi = (CSListing) cachedob;
			} 
			if(poi == null) {
				byte[] bytes = document.getBinaryValue(POI);
				if(bytes != null) {			
					ObjectInputStream in = new ObjectInputStream(new ByteArrayInputStream(bytes));
					poi = (CSListing)in.readObject();
					in.close();
	...
	public static CSListing getCSListingFromCache(int docid, ICache cache) {
		if(cache == null) return null;
		Object cachedob = cache.getObject(""+docid);
		if(cachedob == null) return null;
		return (CSListing) cachedob;
	}
	
	public static Document createAdDocument(CSListing poi) {
		Document document = new Document();
	...
	private static void excludeFromPublishers(CSListing poi, Document document) {
		StringBuffer buffer = new StringBuffer();
	...
	public static Document createLocationDocument(CSListing poi) {
		Document document = new Document();
	...
	public static CSListing createAdCSListing(Document document) {
		try {
